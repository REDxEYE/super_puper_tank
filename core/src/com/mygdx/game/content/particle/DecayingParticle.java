...
public class DecayingParticle extends ParticleV1 implements Alive {
    private final float decaySpeed;
    private float timeToLive;

    public DecayingParticle(Vector2 position, float radius, float timeToLive, float decaySpeed, ImutColor color) {
        super(position);
        this.radius = radius;
    ...
    public void render(ShapeRenderer renderer) {
        var xy = Main.RC.render_objZoom(position);
        renderer.setColor(color.withAlpha(0.1f));
        renderer.circle(xy.x, xy.y, (radius * Main.Zoom), 16);
    }
