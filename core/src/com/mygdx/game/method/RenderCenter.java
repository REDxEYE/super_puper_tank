...
public class RenderCenter {
    ...
    public int ixCam;
    public int iyCam;
    public int ixCamBuff;
    public int iyCamBuff;
    public float width_2 = Main.screenWidth / 2f;
    public float height_2 = Main.screenHeight / 2f;
    public float width_2_zoom = Main.screenWidth / 2f;
    public float height_2_zoom = Main.screenHeight / 2f;
    public float WidthRender = Main.screenWidth, HeightRender = Main.screenHeight;
    public int render_x_max;
    public int render_x_min;
    public int render_y_max;
    public int render_y_min;
    public int block_i_x_max;
    public int block_i_y_max;
    public int render_x;
    public int render_y;
    public int cam_x_width;
    public int cam_y_height;
    public float WidthRenderZoom;
    public float HeightRenderZoom;
    public float WidthRenderZoom2;
    public float HeightRenderZoom2;
    ...
    public float[] render_objZoom(float x, float y) {
        Vector2 tmp = new Vector2(x, y).sub(new Vector2(x2, y2)).scl(Main.Zoom);
        return new float[]{tmp.x, tmp.y};
    }

    public Vector2 render_objZoom(Vector2 pos) {
        return pos.cpy().sub(new Vector2(x2, y2)).scl(Main.Zoom);
    }
...
//                    }
    ...
    public void CameraMapConf() {
        ...
        if (render_x_min < 0) {
            render_x_min = 0;
        } else if (render_x_max > block_i_x_max) {
            render_x_max = block_i_x_max;
        }
        render_y_max = (int) ((y2 + HeightRenderZoom) / Main.height_block + 1);
        render_y_min = (int) ((y2) / Main.height_block - 1);
        if (render_y_min < 0) {
            render_y_min = 0;
        } else if (render_y_max > block_i_y_max) {
            render_y_max = block_i_y_max;
        }
        BuildingConst();
    }
