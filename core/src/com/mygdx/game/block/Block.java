...
import static com.mygdx.game.method.CycleTimeDay.lightFlame;
...
public abstract class Block {
    ...
    public static void LightingAirObject(int xZOOM, int yZOOM, ImutColor RGB, float distance) {
        ...
        for (int i = y_min; i < y_max; i++) {
            for (int i2 = x_min; i2 < x_max; i2++) {
                int gh = (int) sqrt(pow2(xZOOM - AirList.get(i).get(i2).x) + pow2(yZOOM - AirList.get(i).get(i2).y));
                if (gh < distance) {
                    AirList.get(i).get(i2).r += RGB.r * (distance / gh);
                    AirList.get(i).get(i2).g += RGB.g * (distance / gh);
                    AirList.get(i).get(i2).b += RGB.b * (distance / gh);
                    if (AirList.get(i).get(i2).radius == NULL || AirList.get(i).get(i2).radius > gh) {
                        AirList.get(i).get(i2).radius = gh;
