...
import com.mygdx.game.content.Bull.BulletFragment;
...
public abstract class Bullet implements Serializable {
    ...
    protected final void fragments_create() {
        ...
        if (this.time <= 0) {
            ...
            for (int i3 = 0; i3 < this.amount_fragment; i3++) {
                Main.bulletList.add(new BulletFragment(this.x, this.y, this.damage_fragment, this.penetration_fragment, this.type_team));
            }
            this.clear_sost = 1;
        }
    ...
    protected final void spawn_acid() {
        if (1 == rand.rand(3)) {
            acidParticleController.createParticle(new Vector2(this.x, this.y));
        }
    }
    ...
    protected void BullBuildMortar() {
        ...
        if (xM > 0 & yM > 0 & xM < xMap & yM < yMap) {
            if (BlockList2D.get(yM).get(xM).passability & height == 1) {
                ...
                for (int l = 0; l < 20; l++) {
                    Main.bulletList.add(new BulletFragment(this.x, this.y, damage_fragment, penetration_fragment, type_team));
                }
            }
        } else {
            this.clear_sost = 1;
