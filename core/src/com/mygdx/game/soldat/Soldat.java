...
import com.mygdx.game.content.Bull.BulletFlame;
import com.mygdx.game.content.Bull.BulletMortar;
import com.mygdx.game.content.Bull.BullPacket;
import com.mygdx.game.content.Bull.BulletTank;
...
public abstract class Soldat implements Serializable {
    ...
    public void clear(ArrayList<Soldat> obj, int i) {
        if (this.clear_sost == 1) {
            for (int i1 = 0; i1 < 12; i1++) {
                bloodParticleController.createParticle(new Vector2(this.x + i1, this.y));
            }
            obj.remove(i);
        }
    }
    ...
    public void fire_bull(float g) {
        ...
        if (this.time < 0 && abs(g - rotation) < 20) {
            ...
            Main.bulletList.add(new BulletTank(this.x, this.y, -this.rotation + 180, this.damage, this.penetration, this.team, (byte) 1));
    ...
    public void fire_flame(float g) {
        ...
        if (this.time < 0 && abs(g - rotation) < 20) {
            this.time = this.time_max;
            Main.bulletList.add(new BulletFlame(this.x, this.y, -this.rotation + -15 + rand.rand(30) + 180, this.damage, this.t_damage, this.penetration, this.team, (byte) 1));
            Main.bulletList.add(new BulletFlame(this.x, this.y, -this.rotation + -15 + rand.rand(30) + 180, this.damage, this.t_damage, this.penetration, this.team, (byte) 1));
            PacketBull.add(new BullPacket());
            PacketBull.add(new BullPacket());
    ...
    public void fire_mortar(float g) {
        ...
        if (this.time < 0 && abs(g - rotation) < 20) {
            ...
            Main.bulletList.add(new BulletMortar(this.x, this.y, -this.rotation + 180, this.damage, this.penetration, this.damage_fragmentation,
